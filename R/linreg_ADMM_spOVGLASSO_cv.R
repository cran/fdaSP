...
#' Cross-validation for sparse overlap group Least Absolute Shrinkage and Selection Operator
#'
#' Cross-validation for sparse overlap group-LASSO, or sparse group L2-regularized regression, solves the following optimization problem
...
#' Each variable can belong to more than one group.
#' The diagonal matrix \eqn{T\in\mathbb{R}^{p\times p}} contains the variable-specific weights. These values are
...
#' \eqn{\ell_1} sparsity constraint simulatenously and can be obtained by 
...
#' @param var_weights_L1 a vector of length p containing variable-specific weights for the $L_1$ penalty. The default is a vector of ones.
...
#' alpha = 1 is the LASSO penalty, and alpha = 0 the group-LASSO penalty.
...
#' beta <- beta <- c(rep(4, 12), rep(0, p - 13), -2)
...
#' mod_cv <- fdaSP:::linreg_ADMM_spOVGLASSO_cv(X = X, y = y, groups = group1, standardize.data = FALSE, intercept = FALSE,
...
#' confband(xV = log(mod_cv$lambda), yVmin = mod_cv$mse - mod_cv$mse.sd, yVmax = mod_cv$mse + mod_cv$mse.sd)
#' abline(v = log(mod_cv$lambda[which(mod_cv$lambda == mod_cv$lambda.min)]), col = "red", lwd = 1.0)
#' 
#' ### comparison with oracle error
#' mod <- fdaSP:::linreg_ADMM_spOVGLASSO(X = X, y = y, groups = group1, standardize.data = FALSE, intercept = FALSE,
...
#'
#' @noRd
linreg_ADMM_spOVGLASSO_cv <- function(X, Z = NULL, y, groups, group_weights = NULL, var_weights = NULL, var_weights_L1 = NULL, 
                                      standardize.data = TRUE, lambda = NULL, alpha = NULL, lambda.min.ratio = NULL, nlambda = NULL, 
  ...
  if (!check_data_matrix(X)) {
    stop("* linreg_ADMM_spOVGLASSO_cv : input 'X' is invalid data matrix.")  
  }
  if (!check_data_vector(y)) {
    stop("* linreg_ADMM_spOVGLASSO_cv : input 'y' is invalid data vector.")  
  }
  y = as.vector(y)
  # data size
  if (n != length(y)) {
    stop("* linreg_ADMM_spOVGLASSO_cv : two inputs 'X' and 'y' have non-matching dimension.")
  }
  if (!is.null(Z)) {
    if (dim(Z)[1] != length(y)) {
  ...
  if (standardize.data == TRUE) {
    if (intercept == TRUE){
      intercept = FALSE
      warning("* linreg_ADMM_spOVGLASSO_cv : intercept should be set to FALSE is data are standardized!\n")
    }
  }
  ...
  if (!check_param_constant_multiple(c(abstol, reltol))) {
    stop("* linreg_ADMM_spOVGLASSO_cv : tolerance level is invalid.")
  }
  if (!check_param_integer(maxit, 0.0)) {
    stop("* linreg_ADMM_spOVGLASSO_cv : 'maxiter' should be a positive integer.")
  }
  maxiter = as.integer(maxit)
  if (!check_param_constant(rho, 0.0)) {
    stop("* linreg_ADMM_spOVGLASSO_cv : 'rho' should be a positive real number.")
  }
  if (!check_param_constant(mu.ada, 0.0)) {
    stop("* linreg_ADMM_spOVGLASSO_cv : 'alpha' should be a positive real number.")
  }
  if ((tau.ada < 1.0) || (tau.ada > 2.0)) {
    warning("* linreg_ADMM_spOVGLASSO_cv : 'tau' value is suggested to be in [1,2].")
  }
  ...
  if (!check_param_constant(min(lambda), negsmall)) {
    stop("* linreg_ADMM_spOVGLASSO_cv : reg. parameter 'lambda' is invalid.")
  }
  if (min(lambda) < meps) {
    message("* linreg_ADMM_spOVGLASSO_cv : since 'lambda' is effectively zero, a least-squares solution is returned.")
  ...
  # Store
  if (alpha == 0) {
    u_store <- matrix(0L, nrow = sum(nG), ncol = nlambda)
    z_store <- matrix(0L, nrow = sum(nG), ncol = nlambda)
  } else if (alpha == 1) {
    u_store <- matrix(0L, nrow = p, ncol = nlambda)
    z_store <- matrix(0L, nrow = p, ncol = nlambda)
  } else {
    u_store <- matrix(0L, nrow = sum(nG)+p, ncol = nlambda)
    z_store <- matrix(0L, nrow = sum(nG)+p, ncol = nlambda)
  }
  ...
  if (print.out == TRUE) {
    cat("\n\n\n")
    cat("Linear regression model with sparse overlap group-LASSO penalty, Cross Validation\n")
