...
#' provided by the argument \code{var_weights} (see below).
...
#' @param group_weights a vector of length G containing group-specific weights. The default is square root of the group 
#' cardinality, see Bernardi et al. (2022).
...
#' mod_cv <- fdaSP:::linreg_ADMM_OVGLASSO_cv(X = X, y = y, standardize.data = FALSE, intercept = FALSE,
#'                                           groups = list("groups" = group1, "Glen" = Glen),
...
#' mod <- fdaSP:::linreg_ADMM_OVGLASSO(X = X, y = y, standardize.data = FALSE, intercept = FALSE,
#'                                     groups = list("groups" = group1, "Glen" = Glen), nlambda = 30, 
#'                                     control = list("adaptation" = adaptation, "rho" = rho, 
#'                                                    "maxit" = maxit, "reltol" = reltol, "abstol" = abstol, 
#'                                                    "print.out" = FALSE)) 
...
#'
#' @noRd
linreg_ADMM_OVGLASSO_cv <- function(X, Z = NULL, y, groups, group_weights = NULL, var_weights = NULL, standardize.data = FALSE, 
                                    lambda = NULL, lambda.min.ratio = NULL, nlambda = NULL, 
                                    cv.fold = 5, intercept = TRUE, overall.group = FALSE, control = list()) {
  ...
  if (!check_data_matrix(X)) {
    stop("* linreg_ADMM_OVGLASSO_cv : input 'X' is invalid data matrix.")  
  }
  if (!check_data_vector(y)) {
    stop("* linreg_ADMM_OVGLASSO_cv : input 'y' is invalid data vector.")  
  }
  y = as.vector(y)
  # data size
  if (n != length(y)) {
    stop("* linreg_ADMM_OVGLASSO_cv : two inputs 'X' and 'y' have non-matching dimension.")
  }
  if (!is.null(Z)) {
    if (dim(Z)[1] != length(y)) {
  ...
  if (!check_param_constant_multiple(c(abstol, reltol))) {
    stop("* linreg_ADMM_OVGLASSO_cv : tolerance level is invalid.")
  }
  if (!check_param_integer(maxit, 0.0)) {
    stop("* linreg_ADMM_OVGLASSO_cv : 'maxiter' should be a positive integer.")
  }
  if (!check_param_constant(rho, 0.0)) {
    stop("* linreg_ADMM_OVGLASSO_cv : 'rho' should be a positive real number.")
  }
  if (!check_param_constant(mu.ada, 0.0)) {
    stop("* linreg_ADMM_OVGLASSO_cv : 'alpha' should be a positive real number.")
  }
  if ((tau.ada < 1.0) || (tau.ada > 2.0)) {
    warning("* linreg_ADMM_OVGLASSO_cv : 'tau' value is suggested to be in [1,2].")
  }
  ...
  if (!is.null(var_weights)) {
    if (!is.numeric(var_weights)) {
      stop("* linreg_ADMM_OVGLASSO_cv : the vector 'var_weights' is not a numeric vector.")
    }
    if (dim(X)[2] != length(var_weights)) {
      stop("* linreg_ADMM_OVGLASSO_cv : the length of the vector 'var_weights' does not match the number of penalized predictors.")
    }
    if (min(var_weights) < 0.0) {
      stop("* linreg_ADMM_OVGLASSO_cv : the values in 'var_weights' must be positive.")
    }
  }
  ...
  if (!check_param_constant(min(lambda), negsmall)) {
    stop("* linreg_ADMM_OVGLASSO_cv : reg. parameter 'lambda' is invalid.")
  }
  if (min(lambda) < meps) {
    message("* linreg_ADMM_OVGLASSO_cv : since 'lambda' is effectively zero, a least-squares solution is returned.")
  ...
  for (kt in 1:cv.fold) {
    ...
    if (is.null(Z)) {
      ...
      ret <- .admm_ovglasso_cov_fast(W = mX_, Z = mZ_, y = vY_,
  ...
  if (is.null(Z)) {
    ...
    ret  <- .admm_ovglasso_cov(W = X.std, Z = Z, y = y.std, u = t(ret$U[indi.min.mse,]), z = t(ret$Z[indi.min.mse,]),
                               groups = GRmat, group_weights = group_weights, var_weights = var_weights,
  ...
  if (print.out == TRUE) {
    cat("\n\n\n")
    cat("Linear regression model with overlap group-LASSO penalty, Cross Validation\n")
