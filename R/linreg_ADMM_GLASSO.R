...
#' matplot(log(lam), mod$sp.coef.path, type = "l", main = "Group Lasso solution path",
...
#'
#' @noRd
linreg_ADMM_GLASSO <- function(X, Z = NULL, y, groups, group_weights = NULL, var_weights = NULL, standardize.data = TRUE,
                               lambda = NULL, lambda.min.ratio = NULL, nlambda = 30, intercept = FALSE, control = list()) {
  ...
  if (standardize.data == TRUE) {
    if (intercept == TRUE){
      intercept = FALSE
      warning("* linreg_ADMM_GLASSO : intercept should be set to FALSE is data are standardized!\n")
    }
  }
  if (intercept == TRUE) {
    if (!is.null(Z)) {
  ...
  if (!check_data_matrix(X)) {
    stop("* linreg_ADMM_GLASSO : input 'X' is invalid data matrix.")  
  }
  if (!check_data_vector(y)) {
    stop("* linreg_ADMM_GLASSO : input 'y' is invalid data vector.")  
  }
  y <- as.vector(y)
  # data size
  if (dim(X)[1] != length(y)) {
    stop("* linreg_ADMM_GLASSO : two inputs 'X' and 'Y' have non-matching dimension.")
  }
  if (!is.null(Z)) {
    if (dim(Z)[1] != length(y)) {
  ...
  if (!is.null(lambda)) {
    if (length(lambda) == 1) {
      ...
      if (!check_param_constant(lambda, negsmall)) {
        stop("* linreg_ADMM_GLASSO : reg. parameter 'lambda' is invalid.")
      }
      if (lambda < meps){
        message("* linreg_ADMM_GLASSO : since 'lambda' is effectively zero, a least-squares solution is returned.")
  ...
  if (!check_param_constant_multiple(c(abstol, reltol))) {
    stop("* linreg_ADMM_GLASSO : tolerance level is invalid.")
  }
  if (!check_param_integer(maxit, 0.0)) {
    stop("* linreg_ADMM_GLASSO : 'maxiter' should be a positive integer.")
  }
  maxit = as.integer(maxit)
  if (!check_param_constant(rho, 0.0)) {
    stop("* linreg_ADMM_GLASSO : 'rho' should be a positive real number.")
  }
  if (!check_param_constant(mu.ada, 0.0)) {
    stop("* linreg_ADMM_GLASSO : 'mu.ada' should be a positive real number.")
  }
  if ((tau.ada < 1.0) || (tau.ada > 2.0)) {
    warning("* linreg_ADMM_GLASSO : 'tau.ada' value is suggested to be in [1,2].")
  }
  
  # ::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  # Pre-processing: variable weights
  check_weights(x = var_weights, n = dim(X)[2], algname = "linreg_ADMM_GLASSO", funname = "var_weights")
  ...
  # Run lasso
  if (is.null(Z)) {
    ret <- .admm_glasso_fast(A =  X.std, b = y.std, groups = GRmat, group_weights = group_weights, var_weights = var_weights,
    ...
    ret <- .admm_glasso_cov_fast(W = X.std, Z = Z, y = y.std,
    ...
    # ret <- .Call("admm_glasso_cov_fast", W = X.std, Z = Z, y = y.std,
    #              groups = GRmat, group_weights = group_weights, var_weights = var_weights, 
    #              lambda = lambda, rho_adaptation = adaptation, rho = rho, tau = tau.ada, mu.ada,
    #              reltol = reltol, abstol = abstol, maxiter = maxit, ping = 0)
  ...
  if (print.out == TRUE) {
    cat("\n\n\n")
    cat("Linear regression model with group-LASSO penalty\n")
