...
#' Sparse Overlap Group Least Absolute Shrinkage and Selection Operator
#'
#' Sparse overlap group-LASSO, or sparse group L2-regularized regression, solves the following optimization problem
...
#' 
#' @noRd
linreg_ADMM_spOVGLASSO <- function(X, Z = NULL, y, groups, group_weights = NULL, var_weights = NULL, var_weights_L1 = NULL,
  ...
  if (standardize.data == TRUE) {
    if (intercept == TRUE){
      intercept = FALSE
      warning("* linreg_ADMM_spOVGLASSO : intercept should be set to FALSE is data are standardized!\n")
    }
  }
  if (intercept == TRUE) {
    if (!is.null(Z)) {
  ...
  if (!check_data_matrix(X)) {
    stop("* linreg_ADMM_spOVGLASSO : input 'X' is invalid data matrix.")  
  }
  if (!check_data_vector(y)) {
    stop("* linreg_ADMM_spOVGLASSO : input 'y' is invalid data vector.")  
  }
  y <- as.vector(y)
  # data size
  if (dim(X)[1] != length(y)) {
    stop("* linreg_ADMM_spOVGLASSO : two inputs 'X' and 'Y' have non-matching dimension.")
  }
  if (!is.null(Z)) {
    if (dim(Z)[1] != length(y)) {
      stop("* linreg_ADMM_spOVGLASSO : two inputs 'Z' and 'Y' have non-matching dimension.")
    }
  }
  ...
  if (!is.null(lambda)) {
    if (length(lambda) == 1) {
      ...
      if (!check_param_constant(lambda, negsmall)) {
        stop("* linreg_ADMM_spOVGLASSO : reg. parameter 'lambda' is invalid.")
      }
      if (lambda < meps){
        message("* linreg_ADMM_spOVGLASSO : since 'lambda' is effectively zero, a least-squares solution is returned.")
  ...
  if (!check_param_constant_multiple(c(abstol, reltol))) {
    stop("* linreg_ADMM_spOVGLASSO : tolerance level is invalid.")
  }
  if (!check_param_integer(maxit, 0.0)) {
    stop("* linreg_ADMM_spOVGLASSO : 'maxiter' should be a positive integer.")
  }
  maxit = as.integer(maxit)
  if (!check_param_constant(rho, 0.0)) {
    stop("* linreg_ADMM_spOVGLASSO : 'rho' should be a positive real number.")
  }
  if (!check_param_constant(mu.ada, 0.0)) {
    stop("* linreg_ADMM_spOVGLASSO : 'mu.ada' should be a positive real number.")
  }
  ...
  check_weights(x = var_weights, n = dim(X)[2], algname = "linreg_ADMM_spOVGLASSO", funname = "var_weights")
  
  # ::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  # Pre-processing: variable weights (L1)
  check_weights(x = var_weights_L1, n = dim(X)[2], algname = "linreg_ADMM_spOVGLASSO", funname = "var_weights_L1")
  ...
  if (res.method != "ovglasso") {
    warning("* linreg_ADMM_spOVGLASSO : this routine is for fitting the linear regression model with overlap group-LASSO penalty. 
            Use (faster) routine 'linreg_ADMM_spGLASSO' instead or check the provided groups!")
  }
  ...
  if (is.null(group_weights)) {
    ...
    if (overall.group == TRUE) {
      check_group_weights(x = group_weights, n = G, algname = "linreg_ADMM_spOVGLASSO", funname = "group_weights")
      if (length(group_weights) == (G-1)) {
        group_weights <- c(group_weights, sqrt(dim(GRmat)[2]))
      }
    } else {
      check_weights(x = group_weights, n = G, algname = "linreg_ADMM_spOVGLASSO", funname = "group_weights")
    }
  } 
  ...
  # Run lasso
  if (is.null(Z)) {
    ret <- .admm_spovglasso_fast(A = X.std, b = y.std, groups = GRmat,
    ...
    ret <- .admm_spovglasso_cov_fast(W = X.std, Z = Z, y = y.std,
  ...
  if (print.out == TRUE) {
    cat("\n\n\n")
    cat("Linear regression model with sparse overlap group-LASSO penalty\n")
